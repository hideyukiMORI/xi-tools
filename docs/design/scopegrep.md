...
- コメントヒットの所属は「**どの入れ子の中に書かれたか**」であって「誰の説明か」ではない。
  行全体のコメントは、その行のインデントで開いている最も内側のコンテナのパス。行末コメントは、その値のパス。
  🔴 「誰の説明か」は推測しない（`tree-sitter` が直前の兄弟に付けて誤った箇所。「D-2 実測」）。
  ルートに書かれたコメントの JSON Pointer は `""`（RFC 6901 の「文書全体」）
...
| フロー記法 `[a, b]` / `{a: 1}` | **1つのスカラーとして原文のまま持つ**（中に入らない）。**複数行にまたがるときは、内容の各行を1行ずつ別のスカラー行として持つ**（ブロックスカラーと同じ扱い。v1.1・実ファイル計測で最多だったため） |
| タグ `!override` / `!reset` / `!!str` | **読み飛ばす。** タグの後ろの値・入れ子を通常どおり読む（v1.1）。🔴 タグ名は検索に当たらない（値ではないため） |
...
| 構文 | エラー種別 |
| --- | --- |
| アンカー `&a`・エイリアス `*a`・マージキー `<<:` | `Unsupported` |
| 複数行のプレーンスカラー（継続行）・複数行のクォート | `Unsupported` |
| 閉じないフロー記法（`[` や `{` が EOF まで、または入れ子キーの桁より浅い行が来るまでに閉じない） | `Unsupported`（行番号は開いた行） |
...
- `<value>` はヒットした行のスカラーテキスト（原文）。ブロックスカラー・複数行フローなら**その行**だけ
- コメントヒット（`--comments`）は `<file>:<line>: <path> #comment = <# から行末までの原文>`。ルートなら `<path>` が空
...
- `label` が無いときは `null`。8 番目の `kind` は `"value"` か `"comment"`（`--comments` 無しでも常に出る）。キーは常に 8 つ
...
## 実ファイルでの計測（2026-09-02・v1 のバイナリ）

手元の全リポジトリ（`node_modules` / `target` / `dist` / `vendor` / `_work` を除く）の `.yml` / `.yaml` に
v1 を当て、読めたファイル数とエラー種別を数えた。**ファイル名や中身は記録しない。数だけ。**

| | 件数 |
| --- | --- |
| 対象ファイル | 188 |
| 読めた | 169 |
| うち GitHub Actions workflow | **67 / 67**（全部読めた） |
| うち compose | 50 / 66 |
| 読めなかった（自前の fixture 1 件を除く） | 18 |

読めなかった 18 件の内訳:

| 構文 | 件数 | 実態 |
| --- | --- | --- |
| **複数行にまたがるフロー記法** | **14** | compose の `healthcheck.test: [ "CMD-SHELL", … ]` を複数行に割った形。`[` が次の行に来る形が 11、行内で開いて閉じない形が 3 |
| タグ `!override` / `!reset` | 3 | compose の override ファイル |
| `- { $ref: '…' }`（1 行のフローマッピングを要素に持つシーケンス） | 1 | **v1 のバグ。** `{ $ref` を `key: value` の始まりと誤読して「余分な文字」エラーにしていた |
| アンカー / エイリアス | 0 | この corpus には無い |
| 複数ドキュメント `---` | 0 | 同上 |

⇒ **数が指した順**: 複数行フロー → タグ → バグ修正。アンカーと複数ドキュメントは**この計測では要らない**ので、
v1.1 では手を付けない（要るようになったら測ってから）。

**v1.1 のバイナリで同じ 188 ファイルを再計測（同日）**: 読めた **187 / 188**。
読めなかった 1 件は、アンカーを意図的に含む自前の fixture（`scopegrep/testdata/unsupported-anchor.yml`）。
compose は 66 / 66。

実装で決まった細部（設計メモの表より実装が正）:
- 「閉じないまま桁が戻る」検出は**入れ子キーの下でだけ**効く。トップレベルの `a: [` は EOF まで読んでからエラー。
  `<=` にすると `test: [` … `]`（閉じ括弧をキーと同じ桁に置く一般的な書き方）が落ちるため、厳密に浅い（`<`）で判定する
- `key:` の次の行にインデントして置いたフロー記法は、その行で閉じてもよい
- シーケンス要素の先頭が `[` / `{` のときは `key: value` の始まりとして読まない。クォートは特別扱いしない
  （`- "odd key": value` というクォートキーを壊すため）
