...
- 機械強制: **active**（`make coverage`＝`cargo llvm-cov --fail-under-lines`。
  下限は `Makefile` の `COVERAGE_MIN_LINES`。2026-09-02 の実測 92.21% に対して 90 に置いた。
  🔴 下限は**上げる方向にしか動かさない**。下げるのは QLT-005 の ADR 事項）
